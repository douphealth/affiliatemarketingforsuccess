  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (!isPaused && el) {
        ...
        // seamless loop: when halfway through duplicated content, reset
        ...
        >
          {items.map((article, i) => (
            <a
              ...
              className="group flex-shrink-0 w-[340px] p-6 rounded-2xl border border-border bg-card/80 backdrop-blur-sm hover:border-primary/40 hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500 active:scale-[0.97]"
              onMouseEnter={() => setIsPaused(true)}
              onMouseLeave={() => setIsPaused(false)}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-widest uppercase bg-primary/15 text-primary">
                  {article.category}
                </span>
                <span className="text-[11px] text-muted-foreground tabular-nums">{article.date}</span>
              </div>

              <h3 className="text-base font-bold leading-snug group-hover:text-primary transition-colors duration-300 line-clamp-2 mb-4">
                {article.title}
              </h3>
